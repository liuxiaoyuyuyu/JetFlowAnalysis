...
void MyClass::Loop(int job, std::string fList){
    ...
    TH1D* hEvent_Pass   = new TH1D("hEvent_Pass","hEvent_Pass", PUbin,bin0,PUbin);
    TH2D* hPairs[ptbin_A][PUbin];
    TH2D* hNtrig[PUbin];
    TH2D* hNtrigCorrected[PUbin];

    TH1D* hJet_Pass[PUbin];
    TH1D* hJet_Pass550[PUbin];
    TH1D* hJet_Pass550_hltCor[PUbin];
    
    for(int k_PU=0;k_PU<PUbin;k_PU++){
        ...
        hJet_Pass550_hltCor[k_PU]     = new TH1D(Form("hJet_Pass550_hlt_PU_%d",k_PU+1)  ,Form("hJet_Pass550_hlt_PU_%d",k_PU+1)  , trackbin,bin0,trackbin);
    ...
    for(int f = 0; f<fileList.size(); f++){
        ...
        for (Long64_t ievent=0; ievent <nentries; ievent ++){
            ...
            int vtxBool[2] = {0};
            if (nTracksVtx->size() < 24) vtxBool[0] = 1;
            else vtxBool[1] = 1;

            for(int k_PU=0;k_PU<PUbin;k_PU++){
                if(vtxBool[k_PU]) hEvent_Pass->Fill(k_PU);
            } 
            //=================ENTERING JET LOOP==================
            for(int ijet=0; ijet < jetCounter; ijet++){
                ...
                for(int i = 0; i < trackbin; i++){
                    if( n_ChargeMult_DCA_labPt_Eta_exclusion >= trackbinbounds[i] && n_ChargeMult_DCA_labPt_Eta_exclusion < trackbinboundsUpper[i]){
                        ...
                        */
                        ...
                        for(int k_PU=0;k_PU<PUbin;k_PU++){
                            if(vtxBool[k_PU]==1){
                                hJet_Pass[k_PU]->Fill(i);
                                if((*jetPt)[ijet] >= jetPtCut_Event) hJet_Pass550[k_PU]                           ->Fill(i);
                                if((*jetPt)[ijet] >= jetPtCut_Event) hJet_Pass550_hltCor[k_PU]                   ->Fill(i,1.0*jet_HLT_weight);
                            }
                        }
                    }
                }
                ...
                for(int i = 0; i < trackbin; i++){
                    for(int j = 0; j < ptbin_T; j++){
                        for(int k_PU=0;k_PU<PUbin;k_PU++){
                            if(vtxBool[k_PU]==1){
                                hNtrig[k_PU]->Fill(i,j,Ntrig[i][j]);
                                hNtrigCorrected[k_PU]->Fill(i,j, NtrigCorrected[i][j]);
                            }
                        }
                    }
                }
                    ...
                    for(int i = 0; i < trackbin; i++){
                        for(int k_PU=0;k_PU<PUbin;k_PU++){
                            for(int j = 0; j < ptbin_A; j++){
                                if(tkBool[i] + A_ptBool[A_trk][j] +vtxBool[k_PU]== 3){
                            ...
                            for(int j = 0; j < ptbin_T; j++){
                                if(tkBool[i] + T_ptBool[A_trk][j] +vtxBool[k_PU]== 3){
    ...
    TFile* fS_tempA = new TFile(Form("/eos/cms/store/group/phys_heavyions/xiaoyul/Run2_root_out/jTDiff/job_%s.root",subList.c_str()), "recreate");
    for(int wtrk =1; wtrk <trackbin+1; wtrk++){
